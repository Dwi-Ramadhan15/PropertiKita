                ...
                {images.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      currentImage === i ? "w-8 bg-white" : "w-2 bg-white/60"
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-3xl p-7 shadow-sm border border-gray-100">
              ...
              <div className="mt-10">
                <h2 className="text-xl font-bold text-[#1E293B] mb-4">Fasilitas</h2>
                {Array.isArray(item?.fasilitas) && item.fasilitas.length > 0 ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {item.fasilitas.map((nama, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center gap-3 bg-[#F8F8F8] px-4 py-3 rounded-2xl">
                          <span className="text-[#D9AB7B] text-xl">
                            {fasilitasIcon[key] || <MdVerifiedUser />}
                          </span>
                          <span className="text-sm font-medium text-[#1E293B]">{nama}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            {/* MAP WRAPPER DITAMBAHKAN w-full */}
            <div className="mt-8 rounded-3xl overflow-hidden shadow-sm border border-gray-100 w-full relative z-0">
              {/* STYLE DITAMBAHKAN width: "100%" */}
              <MapContainer center={position} zoom={15} style={{ height: "380px", width: "100%" }}>
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                <Marker position={position}>
                  <Popup>
                    <div className="w-52">
                      <img src={images[0]} className="w-full h-24 object-cover rounded mb-2" alt="property" />
          ...
          <div>
            <div className="sticky top-24 bg-white rounded-3xl p-7 shadow-sm border border-gray-100">
              <p className="text-sm text-gray-400 mb-2">Harga Properti</p>
              <h2 className="text-3xl font-bold text-[#1E293B] leading-tight mb-7">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#F8F8F8]">
                <img
                  src={fotoAgen}
                  alt="agen"
                ...
                />
                <div>
                  <p className="font-bold text-[#1E293B]">{agen?.nama_agen || item.nama_agen}</p>
                  <span className="text-xs text-green-600 flex items-center mt-1">
                    <MdVerifiedUser className="mr-1" /> Agen Terverifikasi
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="mt-6 w-full bg-[#22C55E] text-white py-3.5 rounded-2xl font-bold hover:bg-green-600 transition shadow-md flex items-center justify-center"
              >
                <MdWhatsapp className="mr-2 text-xl" />
                Chat WhatsApp
              </button>
