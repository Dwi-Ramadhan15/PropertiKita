...
export default function DaftarPropertiAgen() {
  ...
  const agenId = queryParams.get('agen');

  useEffect(() => {
    const fetchPropertiByAgen = async () => {
      // Jika agenId tidak ada atau string 'undefined', jangan panggil API
      if (!agenId || agenId === 'undefined') {
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="max-w-7xl mx-auto py-16 px-6 min-h-screen bg-[#F8FAFC]">
      <button 
        ...
        className="flex items-center gap-2 text-gray-600 hover:text-blue-600 font-semibold mb-8 transition-all"
      >
      ...
      <div className="mb-12">
        ...
        <p className="text-gray-500">Ditemukan {properti.length} unit dari agen ini</p>
      </div>

      {!agenId || agenId === 'undefined' ? (
        <div className="bg-red-50 p-10 rounded-3xl text-center border border-red-200">
          <p className="text-red-500 font-medium">Error: ID Agen tidak valid.</p>
        </div>
      ) : properti.length === 0 ? (
        <div className="bg-white p-20 rounded-3xl text-center shadow-sm border border-dashed border-gray-300">
          <p className="text-gray-400 text-xl font-medium">Agen ini belum memiliki postingan properti yang disetujui.</p>
        </div>
      ) : (
            ...
            return (
              <div key={p.id} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-gray-100">
                <div className="relative h-64">
                  ...
                  />
                  <div className="absolute top-4 left-4 bg-blue-600 text-white px-4 py-1 rounded-full text-xs font-bold uppercase">
                    {p.tipe}
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-500 text-sm flex items-center gap-2 mb-6">
                    <FaMapMarkerAlt className="text-red-400" /> {p.lokasi}
                  </p>
                  ...
                  <button 
                    ...
                    className="w-full mt-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all"
                  >
                    Lihat Detail
                  </button>
                </div>
